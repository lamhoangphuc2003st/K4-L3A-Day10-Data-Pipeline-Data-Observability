...
| **6** | **Baseline Evaluation & Scoring** | **10** | `src/evaluation/testset.py`, `data/results/baseline_metrics.json`, `data/reports/phase1_report.md` | Theo benchmark PHA 3: sinh 5 câu qua 5 loại (`summary`, `authors`, `date`, `category`, `multi_hop`). Báo cáo Hit Rate thuần vector, Hit Rate hybrid và Token F1 trên dữ liệu sạch. |
