...
| **CP2** | Benchmark Test Set & ChromaDB Vector Store Indexing | 65m - 95m (30') | `src/evaluation/testset.py`, ChromaDB collection `papers-baseline` | Sinh benchmark PHA 3 (5 câu / 5 loại), ChromaDB index 24 docs |
...
- **Nhiệm vụ:**
  ...
  2. Tạo file `.env` từ `.env.example`. Ollama local là mặc định; nếu chọn cloud provider thì điền API key tương ứng.
...
- **Mục tiêu:** Xây dựng benchmark PHA 3 gồm 5 câu thuộc 5 loại và đánh chỉ mục vector trên ChromaDB.
- **Nhiệm vụ:**
  1. Sinh đúng 5 loại câu hỏi: `summary`, `authors`, `date`, `category`, `multi_hop`.
...
- **Tín hiệu nghiệm thu:**
  ...
  ```
  Console in ra `Tín hiệu hoàn thành: Test set gồm 5 câu hỏi`. Smoke test index theo lệnh PHA 3 trong [Guide.md](Guide.md) cần tìm được 2 tài liệu.
...
- **Nhiệm vụ:**
  1. Triển khai 6 kịch bản làm bẩn dữ liệu trong `src/ingestion/corruption.py`:
     - Drop latest records (bỏ 20% bản ghi mới khỏi tập corruption; duplicate scenario giữ tổng số dòng 24).
     - Blank summary (xóa rỗng tóm tắt).
     - Inject text noise (chèn chuỗi rác vào `text_for_embedding`).
     - Truncate title (cắt còn 9 ký tự, dưới 10).
     - Stale date (lùi ngày xuất bản 5 năm).
